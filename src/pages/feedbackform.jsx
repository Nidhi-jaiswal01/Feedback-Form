    ...
    if (user) {
      ...
      const fetchProfile = async () => {
        try {
          ...
          if (matchedProfile) {
            setName(matchedProfile.Name); // assuming your profiles collection has a "Name" field
          }
        } catch (error) {
...
const handleSubmit = async (e) => {
  ...
  const querySnapshot = await getDocs(collection(db, "feedback"));
    ...
    const alreadySubmitted = feedbackList.some((fb) => fb.email === email);

    if (alreadySubmitted) {
      alert("⚠️ Feedback already submitted with this email.");
      return; 
    }
  ...
  const feedbackData = {
    ...
    try {
      ...
      setCategory("");
      setEmail("");
      setFeedback({ foodService: "" });
      setFeedback2({ recognition: "" });
      setFeedback3({ diversity: "" });
      setFeedback4({ leadership: "" });
      setFeedback5({ communication: "" });
